...
namespace Ascii3dEngine
{
    public static class RayTracer
    {
        public static void Trace(bool[,] imageData, Scene scene, Projection projection, List<Actor> actors)
        {
            ...
            double dx = windowWidth / (double)objects.GetLength(0);
            double dy = windowHight / (double)objects.GetLength(1);
            int midX = objects.GetLength(0) / 2;
            int midY = objects.GetLength(1) / 2;
